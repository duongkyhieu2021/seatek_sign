...
class OdooRepository<R extends OdooRecord> {
  ...
  Future<void> unlink(R record) async {
    ...
    await execute(
        recordId: record.id,
        args: [OdooId(modelName, record.id)],
        method: 'unlink');
    await cacheDelete(record);
  }
  ...
  Future<dynamic> execute(
    ...
    await env.queueCall(rpcCall, awaited: awaited).then((result) {
      // Xử lý kết quả trả về
      // log("execute id: $result");
      res = result;
    }).catchError((error) {
      // Xử lý lỗi
      log("lỗi execute Khánh nè $error");
    });
    return res;
  }
}
